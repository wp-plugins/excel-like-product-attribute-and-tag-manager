...
if(isset($plem_settings['wooc_at_fileds'])){
	foreach(explode(",",$plem_settings['wooc_at_fileds']) as $I => $val){
		if($val)
...
if($_REQUEST['DO_UPDATE'] == '1' && strtoupper($_SERVER['REQUEST_METHOD']) == 'POST'){
	...
	foreach($tasks as $key => $task){
	   ...
	   if($parent){
	       foreach($attributes as $attr){
				if(isset($task->{'pattribute_'.$attr->id})){
						 $store = array();
						 $vals = explode(",",$task->{'pattribute_'.$attr->id});
						 foreach($vals as $val){
							if(is_numeric($val)){
						 ...
						 update_post_meta($key, 'attribute_pa_' . $attr->name, implode(',',$store));
				}
		   }
	   }else{ 
		   ...
		   foreach($attributes as $attr){
				if(isset($task->{'pattribute_'.$attr->id})){
						wp_set_object_terms( $key , array_map(fn_correct_type, explode(",",$task->{'pattribute_'.$attr->id})) , 'pa_' . $attr->name );
						
						if(!$task->{'pattribute_'.$attr->id}){
						   if(isset($patts['pa_' . $attr->name]))
							unset($patts['pa_' . $attr->name]);
						}else{
							$patt = null;
							if(!isset($patts['pa_' . $attr->name]))
								$patts['pa_' . $attr->name] = array();
							
							$patt =& $patts['pa_' . $attr->name];
							
							$patt["name"]         = 'pa_' . $attr->name;
							$patt["is_visible"]   = 1;
							$patt["is_taxonomy"]  = 1;
							$patt["is_variation"] = 0;
							
							if(!isset($patt["value"])) $patt["value"]       = "";
							if(!isset($patt["position"])) $patt["position"] = "0";
						}
						
						$patts_set = true;
						//fn_set_meta_by_path($key,"_product_attributes!" . 'pa_' . $attr->name . "!name",'pa_' . $attr->name);
						//fn_set_meta_by_path($key,"_product_attributes!" . 'pa_' . $attr->name . "!is_visible",1);
						//fn_set_meta_by_path($key,"_product_attributes!" . 'pa_' . $attr->name . "!is_taxonomy",1);
				}
		   }
	   ...
	   if(isset($task->tags)){
		  wp_set_object_terms( $key , array_map(fn_correct_type, explode(",",$task->tags)) , 'product_tag' );
	   }
			...
			while (($data = fgetcsv($handle, 8192, ",")) !== FALSE) {
				if($n == 0){
				   ...
				   if( !empty($parent)){
				    ...
				    if($parent){
					   foreach($attributes as $attr){
							if(isset($attribute_indexes[$att->name])){
									 $store = array();
									 $vals = explode(",",$data[$attribute_indexes[$att->name]]);
									 foreach($vals as $val){
										if(is_numeric($val)){
											foreach($attr->values as $trm){
											   if($trm->id == $val){
									 ...
									 update_post_meta($id, 'attribute_pa_' . $attr->name, implode(',',$store));
							}
					   }
				   }else{ 
					   ...
					   foreach($attributes as $att){
							if(isset($attribute_indexes[$att->name])){
								wp_set_object_terms( $id , array_map(fn_correct_type, explode(",",$data[$attribute_indexes[$att->name]])) , 'pa_' . $att->name );
							    ...
							    //fn_set_meta_by_path($id,"_product_attributes!" . 'pa_' . $att->name . "!name",'pa_' . $att->name);
								//fn_set_meta_by_path($id,"_product_attributes!" . 'pa_' . $att->name . "!is_visible",1);
								//fn_set_meta_by_path($id,"_product_attributes!" . 'pa_' . $att->name . "!is_taxonomy",1);
...
?>
<!--
<script src="<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl. 'lib/jquery-2.0.3.min.js' ;?>" type="text/javascript"></script>
-->

<link rel="stylesheet" href="<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl. 'assets/jquery.spreadsheet.full.css';?>">
<script src="<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl. 'assets/jquery.spreadsheet.full.js'; ?>" type="text/javascript"></script>
<!--
...
-->
<!--
<link rel="stylesheet" href="<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl.'handsontable/jquery.handsontable.removeRow.css'; ?>">
<script src="<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl.'handsontable/jquery.handsontable.removeRow.js'; ?>" type="text/javascript"></script>
-->
<link rel="stylesheet" href="<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl.'/lib/chosen.min.css'; ?>">
<script src="<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl.'lib/chosen.jquery.min.js'; ?>" type="text/javascript"></script>

<link rel="stylesheet" href="<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl.'assets/style.css'; ?>">
...
<div id="content-editor" >
	...
	<div class="cmds-editor">
	   <a class="metro-button" id="cmdContentSave" ><?php echo __("Save",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></a>
	   <a class="metro-button" id="cmdContentCancel" ><?php echo __("Cancel",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></a>   
...
<ul class="menu">
  ...
  <li>
   <a class="cmdBackToJoomla" href="<?php echo "admin.php?page=excellikeattributeandtagmanagerforwoocommerceandwpecommercelight-wooc"; ?>" > <?php echo __("Back to Wordpress",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?> </a>
  </li>
  <?php } ?>
  
  <li><span class="undo"><button id="cmdUndo" onclick="undo();" ><?php echo __("Undo",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></button></span></li>
  <li><span class="redo"><button id="cmdRedo" onclick="redo();" ><?php echo __("Redo",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></button></span></li>
  <li>
   <span><span> <?php echo __("Export/Import",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?> &#9655;</span></span>
   <ul>
     <li><span><button onclick="do_export();return false;" ><?php echo __("Export CSV",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></button></span></li>
     <li><span><button onclick="do_import();return false;" ><?php echo __("Update prices from CSV",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></button></span></li>
   </ul>
  </li>
  <li>
   <span><span> <?php echo __("Options",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?> &#9655;</span></span>
   <ul>
     <li><span><button onclick="cleanLayout();return false;" ><?php echo __("Clean layout cache...",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></button></span></li>
   </ul>
  </li>
...
<span class="filters_label" ><span class="toggler"><span><?php echo __("Filters",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></span></span></span>
<div class="filter_holder">
  
  
  <div class="filter_option">
     <label><?php echo __("SKU",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></label>
	 <input placeholder="<?php echo __("Enter part of SKU...",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?>" type="text" name="sku" value="<?php echo $sku;?>"/>
  </div>
  
  <div class="filter_option">
     <label><?php echo __("Product Name",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></label>
	 <input placeholder="<?php echo __("Enter part of name...",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?>" type="text" name="product_name" value="<?php echo $product_name;?>"/>
  </div>
  
  <div class="filter_option">
     <label><?php echo __("Category",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></label>
	 <select data-placeholder="<?php echo __("Chose categories...",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?>" class="inputbox" multiple name="product_category" >
		<option value=""></option>
		<?php
  ...
  <div class="filter_option">
     <label><?php echo __("Tags",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></label>
	 <select data-placeholder="<?php echo __("Chose tags...",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?>" class="inputbox" multiple name="product_tag" >
		<option value=""></option>
		<?php
		    foreach($tags as $tag){
  ...
  <div class="filter_option">
     <label><?php echo __("Shipping class",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></label>
	 <select data-placeholder="<?php echo __("Chose shipping classes...",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?>" class="inputbox" multiple name="product_shipingclass" >
		<option value=""></option>
		<?php
  ...
  <div class="filter_option">
     <label><?php echo __("Product Status",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></label>
	 <select data-placeholder="<?php echo __("Chose status...",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?>"  class="inputbox" name="product_status" multiple >
	    <option value="" ></option>
		<option value="publish"><?php echo __("Published",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></option>
		<option value="pending"><?php echo __("Pending",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></option>
		<option value="draft"><?php echo __("Draft",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></option>
		<option value="auto-draft"><?php echo __("Auto-draft",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></option>
		<option value="future"><?php echo __("Future",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></option>
		<option value="private"><?php echo __("Private",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></option>
		<option value="inherit"><?php echo __("Inherit",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></option>
		<option value="trash"><?php echo __("Trash",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></option>
	 </select>
  </div>
  ...
  ?>	
   <div class="filter_option">
	 ...
	 <select data-placeholder="<?php echo __("Chose values...",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?>" class="inputbox attribute-filter" multiple name="pattribute_<?php echo $att->id; ?>" >
		<option value=""></option>
		<?php
  ...
  <div class="filter_option">
     <input id="cmdRefresh" type="submit" class="cmd" value="<?php echo __("Refresh",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?>" onclick="doLoad();" />
  </div>

  <div class="filter_option">
	  <label><?php echo __("Mass update by filter criteria: ",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></label> 
	  <input style="width:140px;float:left;" placeholder="<?php echo sprintf(__("[+/-]X%s or [+/-]X",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'),'%'); ?>" type="text" id="txtMassUpdate" value="" /> 
	  <button id="cmdMassUpdate" class="cmd" onclick="massUpdate(false);return false;" style="float:right;"><?php echo __("Mass update price",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></button>
	  <button id="cmdMassUpdateOverride" class="cmd" onclick="massUpdate(true);return false;" style="float:right;"><?php echo __("Mass update sales price",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></button>
...
<div class="footer">
 <div class="pagination">
   <label for="txtLimit" ><?php echo __("Limit:",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></label><input id="txtlimit" class="save-state" style="width:40px;text-align:center;" value="<?php echo $limit;?>" plem="<?php $arr =(array)$plem_settings; echo reset($arr); ?>" />
   <?php
   ...
   ?>
   <span class="pageination_info"><?php echo sprintf(__("Page %s of %s, total %s products by filter criteria",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'),$page_no,ceil($count / $limit),$count); ?></span>
   
 </div>
 
 <span class="note" style="float:right;"><?php echo __("*All changes are instantly autosaved",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?></span>
...
window.onbeforeunload = function() {
	...
	if(n > 0){
	  doSave();
	  return "<?php echo __("Transactions ongoing. Plese wait a bit more for them to complete!",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?>";
	}else
	  return;	   
}
...
function massUpdate(update_override){
    if(!jQuery.trim(jQuery('#txtMassUpdate').val())){
	  alert("<?php echo __("Enter value first!",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?>");
	  return;
	} 

	if(confirm("<?php echo __("Update proiduct price for all products matched by filter criteria (this operation can not be undone)?",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight');?>")){
		var POST_DATA = {};
...
jQuery(document).ready(function(){
	...
	customImageEditor.prototype.open = function () {
		...
		jQuery('#TB_iframeContent').load(function(){
			...
			link.setAttribute("href", "<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl; ?>assets/style.css");
			otherhead.appendChild(link);

            var src = doc.createElement("script");
			src.setAttribute("type", "text/javascript");
			src.setAttribute("href", "<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl; ?>lib/script.js");
	...
	$('#dg_wooc').handsontable({
	  ...
	  colHeaders:[
		"ID"
		<?php if(fn_show_filed('name')) echo ',"'.__("Name",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('slug')) echo ',"'.__("Slug",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('sku')) echo ',"'.__("SKU",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('price')) echo ',"'.__("Price",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('override_price')) echo ',"'.__("Sales price",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('categories')) echo ',"'.__("Category",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('featured')) echo ',"'.__("Featured",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('stock_status')) echo ',"'.__("In stock?",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('stock')) echo ',"'.__("Stock",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('tags')) echo  ',"'. __("Tags",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php
		if(fn_show_filed('attributes')){ 
		 foreach($attributes as $att){
		   echo ",".'"'.$att->label.'"'; 
		...
		<?php if(fn_show_filed('status')) echo ',"'.__("Status",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('weight')) echo ',"'.__("Weight",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('height')) echo ',"'.__("Height",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('width')) echo ',"'.__("Width",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('length')) echo ',"'.__("Length",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('image')) echo ',"'.__("Image",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('backorders')) echo ',"'.__("Backorders",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('shipping_class')) echo ',"'.__("Shipp. class",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('tax_status')) echo ',"'.__("Tax status",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		<?php if(fn_show_filed('tax_class')) echo ',"'.__("Tax class",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';?>
		
		<?php
		foreach($custom_fileds as $cfname => $cfield){ 
		   echo ',"'.__($cfield->title,'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight').'"';
		}
        ?>		
	  ...
	  <?php if(fn_show_filed('price')){ ?>,{ data: "price"  ,type: 'numeric',format: '0<?php echo substr($_num_sample,1,1);?>00', readOnly: true}<?php } ?>
	  <?php if(fn_show_filed('override_price')){ ?>,{ data: "override_price"  ,type: 'numeric',format: '0<?php echo substr($_num_sample,1,1);?>00', readOnly: true} <?php } ?> 
...
?>
  ...
  <?php
    if($mu_res){
	   ...
	   ?>
	   jQuery(window).load(function(){
	   alert('<?php echo sprintf(__("Proiduct price for all products matched by filter criteria is changed by %s",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'),$upd_val); ?>');
	   });
	   <?php
	}
	...
	if($import_count){
	   ?>
	   jQuery(window).load(function(){
	   alert('<?php echo sprintf(__("%s products updated prices form imported file!",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'),$import_count); ?>');
	   });
	   <?php
	}
...
function do_import(){
    var import_panel = jQuery("<div class='import_form'><form method='POST' enctype='multipart/form-data'><span><?php echo __("Select .CSV file to update prices/stock from.<br>(To void regular or sales price remove coresponding column from CSV file)",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></span><br/><label for='file'><?php echo __("File:",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></label><input type='file' name='file' id='file' /><br/><br/><button class='cmdImport' ><?php echo __("Import",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></button><button class='cancelImport'><?php echo __("Cancel",'excellikeattributeandtagmanagerforwoocommerceandwpecommercelight'); ?></button></form></div>"); 
...
<script src="<?php echo $excellikeattributeandtagmanagerforwoocommerceandwpecommercelight_baseurl.'lib/script.js'; ?>" type="text/javascript"></script>
</body>
</html>
<?php
exit;
